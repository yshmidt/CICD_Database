...
-- Description:	Created for the Posted A/P Adjustment Transactions 
-- Reports Using Stored Procedure:  glpostApAdj.rpt
...
-- =============================================
CREATE PROCEDURE [dbo].[rptGlPostApAdj]
...
SELECT	GLTRANSHEADER.FY,GLTRANSHEADER.PERIOD ,GLTRANSHEADER.TRANS_NO,GltransHeader.TransactionType,GLTRANSHEADER.POST_DATE,GLTRANSHEADER.TRANS_DT
		...
		,case WHEN gltransheader.TransactionType = 'DM' and gltrans.SourceSubTable = 'APDMDETL' THEN (SELECT CASE WHEN DMEMOS.DMTYPE = 1 THEN cast('Inv# '+RTRIM(dmemos.INVNO)+'  '+'Item #: '+rtrim(apdmdetl.ITEM_NO)+'  '+ rtrim(gltrans.SourceSubTable)+'.'+rtrim(csubidentifier)+':  '+rtrim(cSubDrill) as varchar(80)) 
...
where	gltransheader.TransactionType in('DM','APPREPAY')
