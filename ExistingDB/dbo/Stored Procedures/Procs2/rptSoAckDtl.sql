...
--  04/19/2016 DRP:  This procedure used to be also used for the soackdtl, rmaackdtl also.  But due to complications when the order had Multiple Prices and Multipled Schedules I had to create a separate procedure for the Acknowledgement forms that include the schedule info
--     Created new procedure named <<rptSoAckDtlSch>>
--  05/03/2016 DRP:	 also added HeaderShipto and HeaderShipToAddress to the results
...
--  06/14/2017 DRP:  It was brought to my attention that when there are Sales Discounts and Taxable items involved that the procedure was taking the full Taxable amount it was not calculating the Sales Discount before getting the Tax value. 
...
-- =============================================
		CREATE PROCEDURE [dbo].[rptSoAckDtl] 
...
IF @lFCInstalled = 0
	BEGIN
		select t1.CUSTNO, t1.CUSTNAME,t1.blinkadd, t1.slinkadd,t1.discount, t1.PONO, t1.ORDERDATE, t1.SONO, 
							  ...
							  t1.UOFMEAS,t1.plpricelnk,t1.descriptio, CASE WHEN ROW_NUMBER() OVER(Partition by sono,line_no, plpricelnk Order by orderdate)=1 Then quantity ELSE CAST(0.00 as Numeric(20,2)) END AS Quantity, 
							  ...
							  t1.dscext/100 as dscext,extended-dscext/100 as subtotal2,t1.TAXABLE,t1.FLAT,t1.NOTE,
			...
			--DRP 03/20/2013:  If item was marked as taxable but no tax records existed for that customer a null value would be returned instead of 0.00.  Implemented the isnull(t1.shiptax,0.00) below to address that issue. 
							  -- 12/04/19 VL removed shiptax field, just use soptax and sostax for cube version
			...
			--DRP 03/20/2013:  REPLACED BY ABOVE SupplierAddress.  Place all of the info below into one single Memo field. 		
							 ...
							 -- ,t1.lic_name,t1.LicAddress,t1.AKSTD_FOOT,t1.rma_foot,t1.field149
							  ,t1.saledsctid,t1.ShipToCount,t1.recordtype
							  ,t1.PTaxId, t1.STaxID
		...
		SELECT     TOP (100) PERCENT c1.CUSTNO, c1.CUSTNAME,s1.blinkadd,sd1.slinkadd,s1.SOAMTDSCT as Discount
							  ...
							  i1.PART_CLASS, i1.PART_TYPE, i1.DESCRIPT, sd1.UOFMEAS,sp1.plpricelnk,sp1.descriptio, sp1.QUANTITY, sp1.PRICE, sp1.EXTENDED,
							  ...
							  sp1.TAXABLE, sp1.FLAT, sd1.NOTE,s1.soextend, s1.soptax,s1.sostax,
							  --case when sp1.taxable = '1' and sb1.foreigntax <> '1' then cast (st1.tax_rate*sp1.extended/100 as numeric (12,2)) else cast (0.00 as numeric (12,2)) end as ShipTax,	--06/14/2017 DRP:  Replaced with the below
							  -- 12/04/19 VL removed shiptax field, just use soptax and sostax for cube version
							  --case when sp1.taxable = '1' and sb1.foreigntax <> '1' then cast (round(sp1.EXTENDED*(100-sdc1.DISCOUNT)/100 * st1.TAX_RATE/100,2) as numeric (12,2)) else cast (0.00 as numeric (12,2)) end as ShipTax,
			...
			--04/11/216 DRP:  removed the micssys field below . . will use the GetCompanyAddress on report form. 				 
							 ...
							 ,c1.SALEDSCTID,sc.ShipToCount,@lcShipTo as HeaderShipTo,sp1.recordtype
		...
		FROM         dbo.CUSTOMER as c1 INNER JOIN
							  ...
							  dbo.SOPRICES as sp1 ON sd1.UNIQUELN = sp1.UNIQUELN LEFT OUTER JOIN
...
--10/21/2013 DRP:  I needed to add the "and i2.CUSTNO = plmain.CUSTNO" whern i2 is joined otherwise I was getting every Customer Part number record that existed for the inventory part regardless which customer was selected for the Order. 				
	...
	--04/04/2013 DRP:  added code to filter out cancelled line items. 
		...
		) t1 cross join dbo.shipbill as Hs1 where @lcShipTo = Hs1.LINKADD	--05/03/2016 DRP:  added for the HeaderShipTo information
...
ELSE
	-- FC installed
	BEGIN
		--10/07/16 DRP: Removal Begin:  with the recent changes as to how we get the Currency Symbols we no longer used this below section
				...
				--SELECT @FCurrency = Symbol FROM Fcused WHERE Fcused.Fcused_uniq = fn_GetFunctionalCurrency()	
		--10/07/16 DRP:  removal End
		...
		select t1.CUSTNO, t1.CUSTNAME,t1.blinkadd, t1.slinkadd,t1.discount, t1.PONO, t1.ORDERDATE, t1.SONO, 
			...
			--DRP 03/20/2013:  REPLACED BY ABOVE SupplierAddress.  Place all of the info below into one single Memo field. 		
							  ...
							  t1.dscextFC/100 as dscextFC,extendedFC-dscextFC/100 as subtotal2FC,
			...
			--DRP 03/20/2013:  If item was marked as taxable but no tax records existed for that customer a null value would be returned instead of 0.00.  Implemented the isnull(t1.shiptax,0.00) below to address that issue. 
							  -- 12/04/19 VL removed shiptax field, just use soptax and sostax for cube version
							  --isnull(t1.shiptaxFC,0.00) as shiptaxFC
			...
			--10/07/16 DRP:  Added the values for Presentation
							  ,t1.discountPR, 
		                      CASE WHEN ROW_NUMBER() OVER(Partition by sono,line_no, plpricelnk Order by orderdate)=1 Then pricePR ELSE CAST(0.00 as Numeric(20,2)) END AS pricePR, 
							  CASE WHEN ROW_NUMBER() OVER(Partition by sono,line_no, plpricelnk Order by orderdate)=1 Then extendedPR ELSE CAST(0.00 as Numeric(20,2)) END AS itemExtPR, 
							  t1.dscextPR/100 as dscextPR,extendedPR-dscextPR/100 as subtotal2PR,
							  ...
							  -- 12/04/19 VL removed shiptax field, just use soptax and sostax for cube version
							  --isnull(t1.shiptaxPR,0.00) as shiptaxPR
							t1.ShipToCount,t1.recordtype
							,t1.PTaxId, t1.STaxID
							,hs1.shipto as HeaderShipTo
							,rtrim(hs1.Address1)+case when hs1.address2<> '' then char(13)+char(10)+rtrim(hs1.address2) else '' end+
							  case when hs1.address3<> '' then char(13)+char(10)+rtrim(hs1.address3) else '' end+
		...
		SELECT     TOP (100) PERCENT c1.CUSTNO, c1.CUSTNAME,s1.blinkadd,sd1.slinkadd,s1.SOAMTDSCT as Discount
			...
			--DRP 03/20/2013:  ADDED MICSSYS LICENSE INFO TO THE PROCEDURE, instead of pulling it as a separate table on the reports. 	
							 -- ,micssys.lic_name,rtrim(MICSSYS.LADDRESS1)+case when MICSSYS.LADDRESS2<> '' then char(13)+char(10)+rtrim(MICSSYS.laddress2) else '' end+
							  ...
							  --,micssys.AKSTD_FOOT,micssys.RMA_FOOT,micssys.field149
							  ,c1.SALEDSCTID
							  ,SOAMTDSCTFC as DiscountFC, sp1.PRICEFC, sp1.EXTENDEDFC,		
							  case when sp1.quantity < 0.00 then cast(0.00 as numeric(12,2)) when c1.saledsctid = '' then cast(0.00 as numeric (12,2)) else sp1.extendedFC*sdc1.discount end as dscextFC,   
		                      s1.soextendFC, s1.soptaxFC,s1.sostaxFC,
							  --case when sp1.taxable = '1' and sb1.foreigntax <> '1' then cast (st1.tax_rate*sp1.extendedFC/100 as numeric (12,2)) else cast (0.00 as numeric (12,2)) end as ShipTaxFC	--06/14/2017 DRP:  Replaced with the below. 
							  -- 12/04/19 VL removed shiptax field, just use soptax and sostax for cube version
							  --case when sp1.taxable = '1' and sb1.foreigntax <> '1' then cast (round(sp1.EXTENDEDFC*(100-sdc1.DISCOUNT)/100 * st1.TAX_RATE/100,2) as numeric (12,2)) else cast (0.00 as numeric (12,2)) end as ShipTaxFC
					...
					--10/07/16 DRP:  added the Presentation Values
							   ,SOAMTDSCTPR as DiscountPR, sp1.PRICEPR, sp1.EXTENDEDPR,		
							  case when sp1.quantity < 0.00 then cast(0.00 as numeric(12,2)) when c1.saledsctid = '' then cast(0.00 as numeric (12,2)) else sp1.extendedPR*sdc1.discount end as dscextPR,   
		                      s1.SOEXTENDPR, s1.soptaxPR,s1.sostaxPR,
							  -- 12/04/19 VL removed shiptax field, just use soptax and sostax for cube version
							  --case when sp1.taxable = '1' and sb1.foreigntax <> '1' then cast (st1.tax_rate*sp1.extendedPR/100 as numeric (12,2)) else cast (0.00 as numeric (12,2)) end as ShipTaxPR
							sc.ShipToCount,@lcShipTo as HeaderShipTo,sp1.recordtype
							,@PTaxId AS PTaxId, @STaxId AS STaxID
							,sdc1.DISCOUNT as DiscRate	--06/16/17 DRP:  added
