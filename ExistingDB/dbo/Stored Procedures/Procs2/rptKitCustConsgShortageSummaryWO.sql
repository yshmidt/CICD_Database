...
-- =============================================
	CREATE PROCEDURE [dbo].[rptKitCustConsgShortageSummaryWO]
...
SELECT Wono, Custno, CustName, Prod_no, Prod_Rev, Prod_Desc, Pono, Sono, Part_no, Revision, CustPartNo, CustRev, Descript, 
		ISNULL(SUM(ShortQty),0) AS ShortQty, Part_class, Part_type, AuditDate, Shreason, ISNULL(SUM(Qty),0) AS Qty, Part_sourc, 
		Dept_name, Uniq_key, Dept_id, Kaseqnum
	FROM ZKit
	GROUP BY Wono, Custno, CustName, Prod_no, Prod_Rev, Prod_Desc, Pono, Sono, Part_no, Revision, CustPartNo, CustRev, Descript,
		Part_class, Part_type, AuditDate, Shreason, Part_sourc, Dept_name, Uniq_key, Dept_id, Kaseqnum
UNION ALL
SELECT * FROM ZMiscmain
	ORDER BY CustName, Wono, CustPartNo
end