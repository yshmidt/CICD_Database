...
-- Description:	Created for the Purchased Part Shortage Summary Report by WO Due Date within Kitting
-- Reports Using Stored Procedure:  shrtintw.rpt 
...
--					07/21/16 DRP:  users report an numeric overflow error while attempting to run the report.  I found that the  @ZPoitems declared table had the Balances as numeric(10,2) but the @pur2 table had the balance as numeric(8,2) 
...
-- =============================================
CREATE PROCEDURE [dbo].[rptKitPurchPartShortageWoDueDt]
...
--- 03/28/17 YS changed length of the part_no column from 25 to 35	
---- 10/11/2019 YS change char(25) to char(35) for part_no
Declare @Pur1 table(WONO char(10),Dept_id char(4),Uniq_key char(120), Part_no Char(35), Revision char(35), Descript char(45), Part_Class char(8), Part_type char(8), Part_sourc char(10)
...
declare @NoAVLNeedWoHeader table	(wono char(10),Dept_id char(4),Uniq_key char(10),part_no char(35),Revision Char(8),Descript char(45),part_class char(8)
									,part_type Char(8),Part_sourc char(10),PUniq_key char(10),StdCost numeric(13,5),ProdNo char(35),pRevision char(8)
...
ZNoInvtMfhd	 as (
--				   10/13/14 YS : replaced invtmfhd table with 2 new tables
--				alose use not exists instead of not in
...
-- rename to @pur2
-- 04/14/15 YS Location length is changed to varchar(256)
--07/21/16 DRP:  changed Balance numeric(8,2) to be numeric(10,2)
--- 03/28/17 YS changed length of the part_no column from 25 to 35	
-- 07/13/18 VL changed supname from char(30) to char(50)
-- 07/16/18 VL changed custname from char(35) to char(50)
declare @Pur2 table (WONO char(10),Dept_id char(4),Uniq_key char(120), Part_no Char(35), Revision char(25), Descript char(45), Part_Class char(8), Part_type char(8), Part_sourc char(10)
					,PUniq_key char(10),StdCost numeric(13,5),ProdNo char(35),pRevision char(8), Prod_id char(10),pDescript char(45),Flag char(1),ShortQty numeric(12,2), BomCustno char(10)
					...
					,PoNum char(15),ReqDate smalldatetime,SchdDate smalldatetime,Balance numeric(10,2),PoStatus char(8),PoSupName char(50),Dept_Name char(25),MisType char(1),UniqMfgrHd char(10)
...
insert @Pur2 select  
					...
					-- 07/13/18 VL changed supname from char(30) to char(50)
					-- 07/16/18 VL changed custname from char(35) to char(50)
					P3.*,CAST (00000000.00 as numeric(8,2)),PartMfgr,Mfgr_pt_no,WAREHOUSE,LOCATION,W_KEY,CAST('' as CHAR (15)),cast ('' as smalldatetime) 
					,CAST('' as smalldatetime) as Schd_Date,CAST (00000000.00 as numeric(10,2)) as Balance,CAST('' as CHAR (8)) as PoStatus,CAST(''as CHAR(50)) as PoSupName
...
;
WITH
zUpdInvtAlloc as (SELECT DISTINCT SUM(QtyAlloc) AS QtyRes, W_key, Wono 
					FROM Invt_res 
					WHERE W_key + Wono IN (SELECT DISTINCT W_key+Wono FROM @Pur2) 
					GROUP BY W_key, Wono 
					)
UPDATE @Pur2	
...
--						,Req_date smalldatetime,Balance numeric(10,2),PoSupName char(30),Note1 text,UniqWh char(10),uniqmfgrhd char(10),uniq_key char(10)
--						,CnvtBalance numeric(10,2),nrec integer)
-- 07/13/18 VL changed supname from char(30) to char(50)
CREATE TABLE #ZPoitems (PoNum char(15),CoNum numeric(3),ItemNo char(3),Uniqlnno char(10),postatus char(8),uniqdetno char(10),Schd_date smalldatetime
						,Req_date smalldatetime,Balance numeric(10,2),PoSupName char(50),Note1 text,UniqWh char(10),uniqmfgrhd char(10),uniq_key char(10)
						,CnvtBalance numeric(10,2),nrec integer)
...
-- Now should be ok to start with supplier info (this information is pulled from the Supplier Line Card screen)
...
BEGIN
	...
	IF @@ROWCOUNT > 0
		BEGIN
		SET @str = NULL
-->> 09/24/2012 DRP:  Jeanette brought to my attention that I needed to trim the Supplier Name field so not all of the spaces where included in the resulting report
...
--					,Int_Uniq char(10),Due_date smalldatetime,AVLLink char(10),AvailQty numeric(12,2),PartMfgr char(8),Mfgr_Pt_No char(30),Warehouse char (6),Location varchar (256),w_key char(10)
--					,PoNum char(15),ReqDate smalldatetime,SchdDate smalldatetime,Balance numeric(10,2),PoStatus char(8),PoSupName char(30),Dept_Name char(25),MisType char(1),UniqMfgrHd char(10)
...
-- 01/17/18 VL changed from table variable
SELECT P5.*,LIC_NAME FROM #Purch AS P5 CROSS JOIN MICSSYS ORDER BY Due_date,WONO,Dept_id,Uniq_key,Partmfgr,Mfgr_pt_no
