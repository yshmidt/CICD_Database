...
-- Description:	Created for the Income Statement ~ YTD / Prior Year YTD
-- Reports Using Stored Procedure:  glincst7.rpt
...
-- =============================================
CREATE PROCEDURE [dbo].[rptGlIncStmtYtdPriorYtd]
...
--*****DECLARES THE TABLE WHERE THE YTD TOTALS WILL BE INSERTED*****
	...
	declare @GLYTD as table (Norm_Bal char(2),gl_class char(7),gl_nbr char(13),BegDate smalldatetime,YTDAmt numeric(14,2),PrBegDate smalldatetime,PrYTDAmt numeric (14,2))	
...
--*****DECLARE TABLE WHERE THE FISCAL YEAR DETAIL INFORMATION	*****	
	...
	-- 06/23/15 ys added sequencenumber for prior year
	--06/24/15 YS remove temp table and use variables
...
--*****DECLARES THE TABLE WHERE THE FINAL REPORTS RESULTS WILL BE INSERTED*****
	...
	--Gathers the GL Account detailed information for the YTD and inserts it into the @GLYTD table declared above
			insert	@GLyTD								
					Select	GLTYPES.NORM_BAL,Gl_nbrs.Gl_class,Gl_nbrs.Gl_nbr, null as BegDate,CAST (0.00 as numeric(14,2))as YTDAmt,null as PrBegDate, CAST (0.00 as numeric (14,2)) as PrYTDAmt
	...
	--We had to do this in case they have 13 periods or if the entry is for the first Period of the FY
		...
		--06/24/15 YS remove @fyrs table and use variables to save current fy and period and prior fy and period
		...
		SELECT	@priorFy = FyInfo.priorFiscalYr ,@PriorFyStartDate=FyInfo.PriorFyStartDate,
			...
			FROM @T T LEFT OUTER JOIN  @T T0 ON T.sequenceNumber=T0.sequenceNumber +1 and t.period=t0.period
	...
	--insert @fyrs 
		...
		--where tcurrent.FiscalYr=@lcFy and tprior.Period=@lcPer
...
(
		...
		FROM	GLTRANSHEADER  
				...
				--and gltransheader.period<=f1.PriorPeriod
		...
		wherE gl_nbrs.STMT = 'INC'
				...
				--06/24/15 YS change to use variables, period for prior year is the same as for the current
		...
		insert	@AllTransPr 
			...
			--,@fyrs as F1
			where	JEtype <> 'CLOSE'
			group by FY,GL_NBR,gl_Class
			--,PrFyBegDate order by GL_NBR

	update @GLYTD set PrYTDAmt = (isnull(YTD2.PrPeriodYTDAmt,0.00)) from  @AllTransPr as YTD2,@GLytd as B where YTD2.gl_nbr = B.gl_nbr 
--select * from @GLYTD
	--This below section will calculate the Closing values for Year To Date
			...
			ZYTDClose as
						(
						select	ZYTDClosingNbrs.gl_nbr,SUM(PrYTDAmt) as TotAmt
	...
	update	@GLYTD set PrYTDAmt = ISNULL(zYTDclose.TotAmt,0.00) 
	...
	--This below section will calculate the Totaling value for Year To Date
				;
					...
					ZYTDTotDr as
								(
								select	zYTDTotNbrs.gl_nbr,sum(PrYTDAmt) as TotAmtDr
					...
					zYTDTotCr as
								(
								select	zYTDTotNbrs.gl_nbr,sum(PrYTDAmt) as TotAmtCr
					...
					update	@GLYTD set PrYTDAmt = (ISNULL(zYTDtotdr.TotAmtdr,0.00)) + (ISNULL(zYTDtotcr.totamtcr,0.00))  
					from	ZYTDTotNbrs 
...
(
		...
		wherE gl_nbrs.STMT = 'INC'
				...
				--06/24/15 YS change to use variables, period for prior year is the same as for the current
				 ...
				 and GLTRANSHEADER.period<=@lcPer
				--and F1.pERIOD >= GLTRANSHEADER.period
				--06/23/15 YS optimize the "where" and remove "*"
				and (@lcDiv is null or @lcDiv = '' or PATINDEX('%[0-9][-]'+@lcDiv+'[-]%[0-9]', gl_nbrs.gl_nbr)<>0)
				--and 1 = case when @lcDiv is null OR @lcDiv = '*' or @lcDiv = '' then 1 else  
				--			case when right(LEFT(gl_nbrs.gl_nbr,10),2) = @lcDiv then 1 else 0 END end	--12/10/2014 DRP:  Added this filter to work with the Division parameter.
		) 
		-- 06/24/15 ? YS why do we place prior begin date?
		-- use variable for the beginnig date for the selected FY @SelectedFyStartDate
		insert	@AllTrans
			select	fy,GL_NBR,gl_Class,SUM(Debit-credit) as YTDAmt ,@SelectedFyStartDate as BegDate
			--,PrFyBegDate as BegDate
			from	ZAllTrans
			--,@fyrs as F1
			where	JEtype <> 'CLOSE'
			group by FY,GL_NBR,gl_Class
	...
	--This below section will calculate the Totaling value for Year To Date
				;
					...
					from	ZYTDTotNbrs 
							...
							left outer join zYTDTotCr on ZYTDTotNbrs.gl_nbr = zYTDtotcr.gl_nbr
							left outer join @GLYTD as YTD5 on ZYTDTotNbrs.gl_nbr = YTD5.gl_nbr 
--06/24/15 YS use varaibles
update	@GLYTD 
set		BegDate = @SelectedFyStartDate,PrBegDate = @PriorFyStartDate


--update	@GLYTD 
--set		BegDate = F2.FyBegDate,PrBegDate = f2.PrFyBegDate 
--from	@fyrs as F2
--select * from @GLYTD
--06/24/15 YS use variables
--update	@GLInc1 
--set		FiscalYear=f5.FiscalYear, 
--		Period=F5.Period, 
--		FyBegDate=f5.FyBegDate, 
--		EndDate=f5.PerEndDate, 
--		PrPeriod=f5.PriorPeriod, 
--		PrPeriodFy=f5.PriorFy, 
--		PrEndDate=f5.PrPerEndDate 
--from	@fyrs F5 
update	@GLInc1 
SET FiscalYear= @lcFy,
		Period= @lcper,
		FyBegDate= @SelectedFyStartDate,
...
--			,ytd6.PrYTDAmt,PrBegDate,J.PrPeriodFy,j.PrPeriod,J.PrEndDate,MICSSYS.Lic_name 
--	from	@GLYTD AS YTD6
--			left outer JOIN @GLInc1 AS J ON YTD6.GL_NBR = J.GL_NBR 
--			cross join MICSSYS
--	where ytd6.ytdAmt+ytd6.PrYTDAmt <> 0.00 or J.gl_class ='Title' or J.gl_class = 'Heading'
--	order by gl_nbr
--end
...
--			,ytd7.PrYTDAmt,PrBegDate,K.PrPeriodFy,k.PrPeriod,K.PrEndDate,MICSSYS.Lic_name
--	from	@GLYTD AS YTD7
...
begin
	select	Tot_Start,Tot_End,YTD6.Norm_Bal,GLTYPE,gl_descr,J.gl_class,YTD6.GL_NBR
			...
			,case when J.gl_class = 'Posting' then -ytd6.PrYTDAmt else
			case when J.Norm_Bal = 'DR' and J.gl_class = 'Total  ' then -ytd6.PrYTDAmt else 
				case when J.Norm_Bal = 'CR' and J.gl_class ='Total' then abs(ytd6.PrYTDAmt) else
					case when J.Norm_Bal = 'DR' and J.gl_class  = 'Closing' then ytd6.PrYTDAmt else
						case when J.Norm_Bal = 'CR' and J.gl_class = 'Closing' then -ytd6.PrYTDAmt else
							cast(0.00 as numeric(14,2)) end end end end end as PrYTDAmt
				...
				case when K.Norm_Bal = 'CR' and K.gl_class ='Total' then abs(YTD7.YTDAMT) else
					case when K.Norm_Bal = 'DR' and K.gl_class  = 'Closing' then YTD7.YTDAMT else
						case when K.Norm_Bal = 'CR' and K.gl_class = 'Closing' then -YTD7.YTDAMT else
							cast(0.00 as numeric(14,2)) end end end end end as YTDAMT
			,LONG_DESCR,GLTYPEDESC,FISCALYEAR,PERIOD,FYBEGDATE,ENDDATE
			,case when K.gl_class = 'Posting' then -ytd7.PrYTDAmt else
			case when K.Norm_Bal = 'DR' and K.gl_class = 'Total  ' then -ytd7.PrYTDAmt else 
				case when K.Norm_Bal = 'CR' and K.gl_class ='Total' then abs(ytd7.PrYTDAmt) else
					case when K.Norm_Bal = 'DR' and K.gl_class  = 'Closing' then ytd7.PrYTDAmt else
						case when K.Norm_Bal = 'CR' and K.gl_class = 'Closing' then -ytd7.PrYTDAmt else
							cast(0.00 as numeric(14,2)) end end end end end as PrYTDAmt 
