...
-- Description:	This Stored Procedure was created for the "Project History / SO Detail"
-- Reports:		prjsosh.rpt, prjsosuh.rpt
...
CREATE PROCEDURE [dbo].[rptProjBackOrderHist]
--declare
...
/*RECORD SELECTION*/
-- 09/26/19 YS modified part number/customer part number char(25) to char(35)
declare @tresults table		(PrjGenDt smalldatetime,sono char (10),Line_No char(7),Uniqueln char (10),custno char(10),prjnumber char(10),prjdescrp char(50),prjstatus char(10),Uniq_key char(10)
							,OrdQty numeric (12,5),Balance numeric (12,5),OpenAmtP numeric(12,5), OpenAmtF numeric(12,5),part_no char(35),revision char(8),descript char (45),part_class char(8),part_type char(8)
							,SoExtd numeric(12,5))

;
with	zProjHist as	(	
							select	pjctmain.PRJGENDT,SONO,LINE_NO,sodetail.UNIQUELN,sodetail.uniq_key,ord_qty,BALANCE,CAST (0.00 as numeric (12,5)) as ExtdBal,pjctmain.prjnumber,PJCTMAIN.PRJDESCRP
									,pjctmain.PRJSTATUS,PJCTMAIN.CUSTNO
							from	SODETAIL
									inner join PJCTMAIN on sodetail.PRJUNIQUE = PJCTMAIN.PRJUNIQUE
							where	sodetail.STATUS <> 'Cancel'and sodetail.STATUS <> 'Archived'
									and ORD_QTY > 0
									--and prjnumber like case when @lcProj = '*' then '%' else @lcProj+'%' end	--11/13/15 DRP replaced with below
									and pjctmain.PRJGENDT >= @lcDate
									and exists (select 1 from @tCustomer t inner join customer c on t.custno=c.custno where c.custno=pjctmain.custno)			--11/13/15 DRP:  Added
									and exists (select 1 from @Proj B inner join pjctmain P on B.prjunique=P.prjunique where P.PRJUNIQUE=pjctmain.PRJUNIQUE)	--11/13/15 DRP:  Added

						)
						,
		zPrice as		(
							select	sodetail.UNIQUELN
									,sum(case when soprices.recordtype = 'P' then (price*ORD_QTY)else price*quantity end) as OpenAmtP 
									,CAST (0.00 as numeric (12,5)) as OpenAmtF 
							from	SODETAIL
...
insert @tresults
	select 		zProjHist.prjgendt,zProjHist.sono,zProjHist.LINE_NO,zProjHist.UNIQUELN,zProjHist.CUSTNO,zProjHist.PRJNUMBER,zProjHist.PRJDESCRP,zProjHist.PRJSTATUS,zProjHist.UNIQ_KEY
				,zProjHist.ORD_QTY,zProjHist.BALANCE,zPrice.OpenAmtP, case when zFlatPrice.OpenAmtF IS null then CAST (0.00 as numeric(12,5)) else zFlatPrice.OpenAmtF end as OpenAmtF,PART_NO,REVISION
				,DESCRIPT,PART_CLASS,PART_TYPE,isnull(zPrice.openamtp,0.00)+isnull(zflatprice.openamtf,0.00) as SoExtd
	from		zProjHist
				LEFT OUTER JOIN	zPrice on zProjHist.UNIQUELN = zPrice.UNIQUELN
				left outer join zFlatPrice on zProjHist.UNIQUELN = zFlatPrice.UNIQUELN
				left outer join inventor on zProjHist.UNIQ_KEY = INVENTOR.UNIQ_KEY
