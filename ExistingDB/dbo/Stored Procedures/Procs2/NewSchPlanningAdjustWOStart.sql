...
-- 01/08/19 Shrikant B Added Distinct Before inserting values in @calendar table to avoid primary key voilation errors
-- NewSchPlanningAdjustWOStart '0001213233', '08-01-2019 00:00:00', '08-01-2019 00:00:00' , '49f80792-e15e-4b62-b720-21b360e3108a', 1
-- 06/20/19 YS for the enddate use the first working date prior or =  to the end date, not after the end date 	
-- =============================================
CREATE PROCEDURE [dbo].[NewSchPlanningAdjustWOStart] 
...
BEGIN
	...
	-- 01/08/19 Shrikant B Added Distinct Before inserting values in @calendar table to avoid primary key voilation errors
	INSERT INTO @calendar (ddate, WorkingDay,WorkDayCount) SELECT distinct ddate,WorkingDay,WorkDayCount FROM dbo.fn_GetCalendar(@startDate,@endDate) ORDER BY ddate
	...
	-- 06/20/19 YS use the first working date prior to the end date, not after the end date 	 
	---SELECT @endDate=MIN(cend.dDate) from @calendar cend where dDate>=@endDate and WorkingDay=1
	SELECT @endDate=MAX(cend.dDate) from @calendar cend where dDate<=@endDate and WorkingDay=1
 ...
 SELECT @SumCurrqty = ISNULL(SUM(Curr_qty), 0) from @CurrShopFlView  
...
--03/25/13 YS mistake in calculation of Process_time_h need 24/(Avg_Time_min/60) (was Avg_Time_min/60/24)
		...
		-- 05/30/16 Anuj Changed @nHrsProcessTime numeric(7,3) to @nHrsProcessTime numeric(9,3) as it is causing error numeric to numeric data type casting
		-- 05/30/16 Anuj Changed @nRemainingTime data type to int as it will always be int instead of numeric(7,3)
		declare @nHrsProcessTime numeric(9,3),@nNotWorking int,@nRemainingTime int,@nslack numeric(9,3),@modifypri int=1,@nslackpri int
  SELECT @nHrsProcessTime= ISNULL(SUM(Capctyneed/3600.00), 0) FROM @CurrShopFlView  
		...
		SELECT SF.*,Process_time_h +ISNULL(l.wcSlack,0) as NewProcessHrs,  
			...
			select * from DEPT_QTY where WONO=@wono
			COMMIT			
			--select * from SFDueOut order by number
			-- re-set to default
