...
-- =============================================
CREATE PROCEDURE [dbo].[rptPkHisAmtbyPk] 
--06/08/2011 ~ Deb: added the Date Range and Posted status parameters to speed the response time on larger datasets 
--06/08/2011 ~ Deb:	Also had to make the Stored Procedure more unique to each report because of these parameters.  
--					It used to have pkhisamt.rpt, pkhis_p.rpt and pkhis_so.rpt also using this SP, but now they will more than likely have to have their own.
...
IF @lFCInstalled = 0
	...
	Where		PLMAIN.SHIPDATE>=@lcDateStart and PLMAIN.SHIPDATE < @lcDateEnd+1
				and plmain.printed = case when @lcStatus = 'Unposted' then 0 else 1 end 
...
-- FC istalled	
	...
	select		t1.CUSTNO,t1.CUSTNAME, t1.STATUS, t1.Sono, t1.SHIPDATE, t1.packlistno, t1.INVOICENO, t1.PONO, t1.line_no,t1.sortby, t1.PART_NO, t1.REVISION, t1.PART_CLASS, t1.PART_TYPE,
				...
				, t1.Fcused_uniq AS Fcused_uniq--, t1.Currency AS Currency
