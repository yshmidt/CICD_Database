...
-- Description:	Created for the Income Statement ~ Current Period / Prior Period
-- Reports Using Stored Procedure:  glincst3.rpt
...
-- =============================================
CREATE PROCEDURE [dbo].[rptGlIncStmtCurrPriorPd]
...
--***PRIOR PERIOD***--	
	...
	--This section will calculate the Closing Value for Prior Period	
		;
		with
		ZPClosingNbrs as
					(				
		...
		--This below section will calculate the Totaling value for Prior Period
			;
				with
				ZPTotNbrs as
							(				
...
--This below section will calculate the Total value for Period/Fiscal Year Selected			
	...
	from @fyrs as F1 cross join MICSSYS
...
if (@lcShowAll = 'No')
	...
	select	gb1.tot_start,gb1.tot_end,gb1.Norm_Bal,gb1.GLTYPE,gl_descr,gb1.gl_class,gb1.GL_NBR
					...
					case when GB1.Norm_Bal = 'CR' and GB1.gl_class ='Total' then abs(Amt) else
						case when GB1.Norm_Bal = 'DR' and GB1.gl_class  = 'Closing' then Amt else
							case when GB1.Norm_Bal = 'CR' and GB1.gl_class = 'Closing' then -Amt else
								cast(0.00 as numeric(14,2)) end end end end end as Amt
			,long_descr,glTypeDesc,FiscalYear,Period,EndDate
