...
-- Description: Change Mfgr_pt_no values in all tables (system utility)
-- 04/14/15 YS Location length is changed to varchar(256)
-- =============================================
CREATE PROCEDURE [dbo].[sp_ChangeMfgr_pt_no] @lcOldMfgr_pt_no char(30) = ' ', @lcNewMfgr_pt_no char(30) = ' ', @lcOldPartMfgr char(8) = ' ', 
	@lcNewPartMfgr char(8) = ' ', @lChgPartMfgr bit = 0, @lcUserId char(8) = ' '
AS
BEGIN
...
DECLARE @ZOldNewDupl TABLE (nRecno int, Uniq_key char(10), Partmfgr char(8), N int)
-- 04/14/15 YS Location length is changed to varchar(256)
...
DECLARE @lnTableVarCnt int, @lnTotalCount int, @lnCnt int, @lcOldUniqMfgrHd char(10), @lcNewUniqMfgrHd char(10), @lcUniq_key char(10), 
		@lcPartmfgr char(8), @lnTotalCount2 int, @lnCnt2 int, @lcLocOldW_key char(10), @lcLocNewW_key char(10), @lnLocQty_oh numeric(12,2),
...
-- First get all invtmfhd records that have either old mfgr_pt_no or new mfgr_pt_no, depends on if need to change partmfgr too
INSERT @ZNewOldMfg (Uniq_key, Partmfgr, Mfgr_pt_no, UniqMfgrhd, Is_Deleted, MatlType)
		...
		FROM InvtMfhd
		WHERE 1 = CASE WHEN @lChgPartMfgr = 1 THEN 
					CASE WHEN (Partmfgr = dbo.PADR(@lcOldPartMfgr,8,' ') OR Partmfgr = dbo.PADR(@lcNewPartMfgr,8,' ')) THEN 1 ELSE 0 END
					ELSE CASE WHEN Partmfgr = dbo.PADR(@lcOldPartMfgr,8,' ') THEN 1 ELSE 0 END END
		AND (UPPER(MFGR_PT_NO) = UPPER(dbo.PADR(@lcOldMfgr_pt_no,30,' ')) OR UPPER(MFGR_PT_NO) = UPPER(dbo.PADR(@lcNewMfgr_pt_no,30,' ')))
...
INSERT @ZOldNewDupl (Uniq_key, Partmfgr, N)
	SELECT Uniq_key, Partmfgr, COUNT(*) AS N
		FROM @ZNewOldMfg ZnewoldMfg
		GROUP BY Uniq_key, Partmfgr
		HAVING COUNT(*) > 1
...
BEGIN
	...
	BEGIN
	SET @lnCnt = @lnCnt + 1	
	SELECT @lcUniq_key = Uniq_key, @lcPartmfgr = Partmfgr FROM @ZOldNewDupl WHERE nRecno = @lnCnt;
	IF @@ROWCOUNT > 0
		BEGIN
			-- Old Uniqmfgrhd
			SELECT @lcOldUniqMfgrHd = ISNULL(UniqMfgrHd,SPACE(10))
				...
				AND Mfgr_pt_no = @lcOldMfgr_pt_no
			...
			SELECT @lcNewUniqMfgrHd = ISNULL(UniqMfgrHd,SPACE(10))
				...
				WHERE Uniq_key = @lcUniq_key
				AND PartMfgr = @lcPartmfgr
				AND Mfgr_pt_no = @lcNewMfgr_pt_no
...
INSERT @ZUpdTable
	SELECT O.Name AS TableName, C.Name AS FieldName 
		FROM sys.all_objects O, sys.all_columns C
		WHERE O.Object_id = C.Object_id
		AND (LTRIM(RTRIM(C.Name)) = 'mfgr_pt_no'
		OR CHARINDEX('mfgr_pt_no',c.name) > 0 )
		AND Type = 'U'
		ORDER BY Tablename
IF @@ROWCOUNT > 0
	BEGIN
	-- This sp will go through @ZUpdTable table all table and field to update from old partmfgr to new ones
	SELECT @lcUpdString = 'Partmfgr = '''+@lcOldPartMfgr + ''' AND LOWER(Mfgr_pt_no) = ''' + LOWER(@lcOldMfgr_pt_no)+''''
	EXEC dbo.[sp_GlobalUpdateField] @ZUpdTable, @lcNewMfgr_pt_no, @lcUpdString
END		
