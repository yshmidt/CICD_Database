...
-- Description:	<Compiles the details for the Shipping Report by Packing List>
-- Used On:     <Crystal Report {PkByPk.rpt}>
...
--				08/30/16 DRP:  needed to update the Description to make sure that it pulled fwd the information for when the items were misc. 
-- =============================================
CREATE PROCEDURE [dbo].[rptPkByPkWM]
		--DECLARE
...
SELECT	plmain.PACKLISTNO,plmain.CUSTNO,CUSTNAME,plmain.SONO,SHIPDATE,somain.pono,plmain.SHIPVIA,plmain.WAYBILL,plmain.LINKADD,SHIPTO
		...
		--,ISNULL(cast(inventor.descript as CHAR(45)),CAST(pldetail.cdescr as CHAR(45))) as Descript	--08/30/16 DRP:  replaced with the below
...
where	plmain.PRINTED = 1
		--and plmain.SHIPDATE>=@lcDateStart AND plmain.SHIPDATE<@lcDateEnd+1	--01/06/2015 DRP Remove
