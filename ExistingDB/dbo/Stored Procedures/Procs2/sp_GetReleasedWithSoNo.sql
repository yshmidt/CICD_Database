...
-- Description:	get packing list with Sales order number
--[dbo].sp_GetReleasedWithSoNo '28', 0,50,'',''
...
-- =============================================
CREATE PROCEDURE [dbo].sp_GetReleasedWithSoNo --0000000028
	@sono AS nvarchar(10) = null,
	@startRecord int,
	@endRecord int, 
	@sortExpression nvarchar(1000) = null,
	@filter nvarchar(1000) = 'PackListNo asc'
AS
...
;WITH packingList AS(	
	...
	AND CHARINDEX(@sono ,UPPER(PLMAIN.SONO))>0 AND PRINTED=1 
	)
--Sachin S-05-16-2016:Get the filtered list based on the column and fifty records at a time
