...
-- Description:	this procedure will be called from the SF module for getting Serialized WO List by WC
-- GETWCSerializedWO 'STAG'
-- =============================================
CREATE PROCEDURE GETWCSerializedWO
 @wcName CHAR(4)
...
BEGIN
	...
	t1.NUMBER,ITAR,OutSourced--,COUNT(j.JBSHPCHKUK)  AS PDMCount
	,ISNULL(WOSerialStartNumber,0) AS WOSerialStartNumber,d.DEPT_ID AS FromWC,d.DEPTKEY AS FromDeptKey
	FROM #TEMP1 t1 
	INNER JOIN DEPT_QTY d ON t1.Wono = d.WONO AND t1.NUMBER =d.NUMBER
	INNER JOIN INVTSER ser ON ((d.DEPTKEY =ser.ID_VALUE AND ID_KEY ='DEPTKEY'  AND t1.Wono = ser.WONO) OR (ID_KEY ='W_KEY'  AND t1.Wono = ser.WONO AND d.DEPT_ID = @wcName))
	...
	ISNULL(WOSerialStartNumber,0) AS WOSerialStartNumber,d.DEPT_ID AS FromWC,d.DEPTKEY AS FromDeptKey
	FROM #TEMP1 t1 
	INNER JOIN DEPT_QTY d ON t1.Wono = d.WONO AND t1.NUMBER =d.NUMBER AND d.SERIALSTRT=1
	LEFT OUTER JOIN JBSHPCHK j ON j.WONO = d.WONO AND j.DEPTKEY =d.DEPTKEY AND j.isMnxCheck =1 AND j.CHKFLAG =1
	WHERE WOSerialStartNumber IS NOT NULL
	GROUP BY RowNum,t1.SERIALSTRT,KITCOMPLETE,[Priority],[Description],UniqKey,t1.Wono,Revision,PartNo,PartNoWithRev,WoQty,BALANCE,WoDueDate,SERIALYES,LOTDETAIL,WcDueDate,[Status],
	CUSTNAME,t1.UNIQUEREC,Assigned,t1.equipment,ProcessTime,TotalTime,WcQty,WoProcessTime,AUTODT,FGIEXPDAYS,SetupTime,ORDMULT,UseIpKey,t1.NUMBER,ITAR,OutSourced,
