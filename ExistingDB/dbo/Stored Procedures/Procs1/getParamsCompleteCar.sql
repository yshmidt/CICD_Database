...
-- Description:	procedure to get list of Completed Corrective Action Request list
-- Modifaction: 06/06/17 DRP:  removed the code that removed the leading zeros so it will sort on screen properly
-- =============================================
CREATE PROCEDURE [dbo].[getParamsCompleteCar] 
...
BEGIN
    ...
    -- Insert statements for procedure here
   	if (@top is not null)
		...
		--select	carno as Value, SUBSTRING(carno+'     '+PROB_TYPE,PATINDEX('%[^0]%',carno + ' '),LEN(carno+PROB_TYPE)) AS Text 
		from	CRACTION 
		WHERE	COMPDATE is not null
				and 1 = case when @paramFilter is null then 1 else case when CARNO like '%'+@paramFilter+ '%' then 1 else 0 end end
		ORDER BY carno
