...
-- Description: Get resered manufacturers data for over reserved grid    
-- Modification     
...
-- EXEC GetReservedMpnData '0000001328','KX7EY3JI7B','','','','WoNoDept desc',''    
-- =============================================    
CREATE PROCEDURE GetReservedMpnData    
(    
@wono AS CHAR(10),    
@uniqKey AS CHAR(10)='',     
@uniqWH AS CHAR(10)='',    
...
BEGIN    
 ...
 SELECT DISTINCT identity(int, 1, 1) as Number, INVT.uniqmfgrhd AS UniqMfgrHd     
    ...
    ,IM.W_Key AS WKey    
    ,ISNULL(IR.Allocated,0) AS AllReserved     
    ,(IM.Qty_Oh- IM.Reserved) AS QtyOh    
   ...
   INNER JOIN InvtResCte IR ON IR.W_KEY = IM.W_KEY AND IR.UNIQ_KEY = IM.UNIQ_KEY AND IR.KaSeqnum = k.KASEQNUM    
 ...
 WHERE (@uniqKey IS NULL OR @uniqKey = '' OR IM.UNIQ_KEY = @uniqKey)    
    ...
    AND IR.WONO != @wono AND IR.Allocated > 0    
