...
-- Description:	this procedure will be called from the SF module and get all the Serial data which are associated with this warehouse/Lot
-- 10/19/16 Sachin b Removing the Leading zeros from serial no
-- 03/21/17 Sachin b Add inner query for the serialunique
-- [dbo].[GetWareHouseSerialData] '_2730WADFA','_2730WBGIZ','_2730XLW78',true,1,3000,'',''
--02/09/18 YS changed size of the lotcode column to 25 char
-- =============================================

CREATE PROCEDURE [dbo].[GetWareHouseSerialData] 
...
if(@IsLotted = 0)
	BEGIN
		...
		AND imfgr.INSTORE = 0
		-- 03/21/17 Sachin b Add inner query for the serialunique
		AND ser.SERIALUNIQ not in (select SERIALUNIQ from iReserveSerial where isDeallocate =0 and serialuniq not in (select SERIALUNIQ from iReserveSerial where isDeallocate =1 ))
	END
ELSE
    BEGIN
