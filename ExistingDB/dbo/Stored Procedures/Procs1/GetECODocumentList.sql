...
-- Modified Vijay G:  Change Join filter condition
-- GetECODocumentList 'XALWXZCM7Y',0,150,null,null    
-- =============================================      
      
CREATE PROCEDURE GetECODocumentList       
@uniqECNo CHAR(10),        
@StartRecord INT =0,      
@EndRecord INT =150,       
...
DECLARE @ParentId CHAR(10) = (    
        SELECT TOP 1 fc.FileId FROM ECMAIN ec    
        INNER JOIN WmFileRelationShips re ON ec.UNIQECNO =re.RecordId AND RecordType ='ECODocuments'    
        INNER JOIN WmFileCabinet fc ON 'ECO_'+ ec.ECONO = fc.[filename] -- Modified Vijay G:  Change Join filter condition   
        WHERE ec.UNIQECNO =@uniqECNo    
        )    
