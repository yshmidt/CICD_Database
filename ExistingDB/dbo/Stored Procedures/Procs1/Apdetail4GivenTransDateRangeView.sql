...
CREATE PROCEDURE [dbo].[Apdetail4GivenTransDateRangeView] 
...
BEGIN
	...
	IF @lFCInstalled = 0
		BEGIN
		IF (@lcApSatus='') 
			SELECT Item_no, Item_desc, Qty_each, Price_each, Item_Total, Is_Tax, ISNULL(TaxTabl.Taxdesc, SPACE(25)) AS Taxdesc, ISNULL(ApdetailTax.Tax_rate,0) AS Tax_rate, Gl_nbr, Apdetail.UniqApHead ,Trans_dt,Apdetail.UniqApdetl
	...
	ELSE
		BEGIN
		IF (@lcApSatus='') 
			SELECT Item_no, Item_desc, Qty_each, Price_eachFC, Item_TotalFC, Price_each, Item_Total, Is_Tax, ISNULL(TaxTabl.Taxdesc, SPACE(25)) AS Taxdesc, ISNULL(ApdetailTax.Tax_rate,0) AS Tax_rate, Gl_nbr, Apdetail.UniqApHead ,Trans_dt,Apdetail.UniqApdetl
				FROM Apmaster, Supinfo, Apdetail LEFT OUTER JOIN ApdetailTax ON Apdetail.UniqApdetl = ApdetailTax.UniqApdetl
				LEFT OUTER JOIN Taxtabl ON ApdetailTax.Tax_id = Taxtabl.Tax_id
				where supinfo.uniqsupno=apmaster.uniqsupno 
				AND Apmaster.UniqAphead = Apdetail.UniqAphead
